...
ggplot(ungulate, aes(species, length.exposure))+ geom_boxplot()+xlab("Species")+ ylab("Days in ground")+
  theme_minimal()

#standardize time
ungulate$length.exposure <-scale(ungulate$length.exposure)
...
trttime.trtxtime <- glmer(survival~length.exposure*trt + (1|site), family = binomial, data=ungulate)
#this is repeated in the sequence below, within the list of models requested by the editor
trtspp.trtxspp <- glmer(survival~species*trt + (1|site), family = binomial, data=ungulate)
#this is repeated in the sequence below, within the list of models requested by the editor
aictab(list(timetrt, spptrt), modnames=c("trttime.trtxtime", "trtspp.trtxspp"))


###Three main models
trttimespp.trtx <- glmer(survival ~ trt + length.exposure + species + trt:length.exposure + species:trt + (1|site), family = binomial, data = ungulate)
trtspp.trtxspp <- glmer(survival ~ trt + species + species:trt + (1|site), family = binomial, data = ungulate)
trttime.trtxtime <- glmer(survival ~ trt + length.exposure + trt:length.exposure + (1|site), family = binomial, data = ungulate)

aictab(list(trttimespp.trtx, trtspp.trtxspp, trttime.trtxtime))

###All models listed in editor request###

#Treatment
trt <- glmer(survival ~ trt + (1|site), family = binomial, data = ungulate)

#Species
spp <- glmer(survival ~ species + (1|site), family = binomial, data = ungulate)

#Time
time <- glmer(survival ~ length.exposure + (1|site), family = binomial, data = ungulate)

#Treatment Species
trtspp <- glmer(survival ~ trt + species + (1|site), family = binomial, data = ungulate)

#Treatment Time
trttime <- glmer(survival ~ trt + length.exposure + (1|site), family = binomial, data = ungulate)

#Species Time
spptime <- glmer(survival ~ species + length.exposure + (1|site), family = binomial, data = ungulate)

#Treatment Species Time
trtspptime <- glmer(survival ~ trt + species + length.exposure + (1|site), family = binomial, data = ungulate)

#Treatment Species Treatment*Species
trtspp.trtxspp <- glmer(survival ~ trt + species + trt:species + (1|site), family = binomial, data = ungulate)
...
spptime.sppxtime <- glmer(survival ~ species + length.exposure + species:length.exposure + (1|site), family = binomial, data = ungulate)

#Treatment Species Time Treatment*Species
trtspptime.trtxspp <- glmer(survival ~ trt + species + length.exposure + trt:species +(1|site), family = binomial, data = ungulate)

#Treatment Species Time Treatment*Time
trtspptime.trtxtime <- glmer(survival ~ trt + species + length.exposure + trt:length.exposure + (1|site), family = binomial, data = ungulate)

#Treatment Species Time Species*Time
trtspptime.sppxtime <- glmer(survival ~ trt + species + length.exposure + length.exposure:species + (1|site), family = binomial, data = ungulate)

#Treatment Species Time Treatment*Species Treatment*Time
trtspptime.trtx <- glmer(survival ~ trt + species + length.exposure +trt:species + trt:length.exposure  +(1|site), family = binomial, data = ungulate)

#Treatment Species Time Treatment*Species Species*Time
trtspptime.sppx <- glmer(survival ~ trt + species + length.exposure +trt:species + species:length.exposure +(1|site), family = binomial, data = ungulate)

#Treatment Species Time Treatment*Time Species*Time
trtspptime.timex <- glmer(survival ~ trt + species + length.exposure +trt:length.exposure + species:length.exposure +(1|site), family = binomial, data = ungulate)

#Treatment Species Time Treatment*Species Treatment*Time Species*Time
full <- glmer(survival ~ trt + species + length.exposure + trt:species + trt:length.exposure + species:length.exposure +(1|site), family = binomial, data = ungulate)
