...
###### Just pigs ##############
mpigtot<-lm(totalsdl~pig, vegandsign)
mpigtot2<-lm(totalsdl~pig, vegandsign2)
summary(mpigtot)

mpignat<-lm(nativsdl~pig, vegandsign)
mpignat2<-lm(nativsdl~pig, vegandsign2)
summary(mpignat)
summary(mpignat2)

mpignnat<-lm(nonnativesdl~pig, vegandsign)
mpignnat2<-lm(nonnativesdl~pig, vegandsign2)
summary(mpignnat)
summary(mpignnat2)

mpigvin<-lm(vines~pig, vegandsign)
mpigvin2<-lm(vines~pig, vegandsign2)
summary(mpigvin)
summary(mpigvin2) # p = 0.07

###r-squared values for input###
rtotpig<-summary(lm(totalsdl~pig, vegandsign))$r.squared
##[1] 0.001490327
rnatsdlpig<-summary(lm(nativsdl~pig, vegandsign))$r.squared
##[1] 0.002768039
rnonsdlpig<-summary(lm(nonnativesdl~pig, vegandsign))$r.squared
##[1] 0.02305202
rvinsdlpig<-summary(lm(vines~pig, vegandsign))$r.squared
##[1] 0.001429389
...
p1<- ggplot(data=vegandsign, aes(y=totalsdl, x=pig))+
  ...
  annotate("text", x = 0.75, y = 800, label = r01, parse=T)
